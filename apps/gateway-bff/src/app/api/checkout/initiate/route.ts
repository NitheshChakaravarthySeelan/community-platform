...
const kafka = new Kafka({
  ...
  brokers: [process.env.KAFKA_BOOTSTRAP_SERVERS || 'localhost:29092'], // Use env var or default
});
...
// Ensure producer is connected before handling requests
connectProducer();
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId || !items || !Array.isArray(items) || items.length === 0 || !totalAmount) {
      return NextResponse.json({ error: 'Missing required fields: userId, items, totalAmount' }, { status: 400 });
    }
    ...
    await producer.send({
      topic: 'checkout.checkout-events', // Send to our central event topic
      messages: [
        {
